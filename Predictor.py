...
st.title(
    """
               2OM-Pred: Predictor for 2-O-Methylation Sites
"""
)
st.subheader("""2-O-methylation (2OM) refers here to the creation of a new functional group through the attachment of a methyl (-CH3) group to the second position of aromatic ring hydroxyl group (-OH). It plays an active part of RNA’s physical configuration stability and the way; different RNA molecules interrelate. Further, this modification plays a pivotal role in changing epigenetic regulation of cellular process. Previous approaches like the mass spectrometry could not fully enhance the identification of RNA modified sites. Sequence data was useful in the development of measures that meant that the use of computationally intelligent system to quickly identify 2OM sites. This research proposed a new method of features generation through matrices and vectors as discussed in the ensuing sub-sections. The features dimensionality reduction has been done through statistical moments criteria. The final obtained features were used to train ensemble models. To assess the ensemble models, two benchmark measures, namely independent set test and k-fold cross validation were employed. Through rigorous testing, bagging ensemble model outperformed and revealed optimal accuracy scores.""")
#---------------------------------#
image = Image.open('Figure_01.png')
st.image(image)
    ...
    if st.button("Submit"):
        ...
        for i in range(lno):
            ...
            if output_proba[i] > 0.7:
                st.info("Output = 2-O-M Site")
            else:
                st.info("Output = Non 2-O-M Site")
